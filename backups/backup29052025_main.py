...
def ret_of_type(val: Any,
                type_check: Any,
                err_str: str="Incorrect input! ") -> Any:
    """
    ...
    :param err_str: an input string during the value request
...
def ret_valid(min_buf: Any,
    ...
    :return: value of type type_check
    """
    buf = -1
    while not min_buf < buf < max_buf:
        value: str | type = input(inp_str)
        buf = ret_of_type(value, type_check, f"Wrong type! Expected type: {type_check} >>> ")
    return buf
...
def option_menu() -> str | None:
    ...
    try:
        ...
        print("That is not an option! Choose between 1 and 2")

def new_coords() -> tuple:
    """
...
def team_turn(option: str,
    ...
    if option == "1":
        ...
        if board.get_occupancy_board()[new_x][new_y] == "0":
            team.unit_list_inst[unit_id].move(new_coords()[0],
                                              new_coords()[1],
                                              board.board_size)
            return
        ...
        if board.get_occupancy_board()[attack_x][attack_y] not in f"{team.symbol}0":
